...
# A list of the the running local jobs so we can find them to top them if needed
local_jobs = []

# Save the origional TTY Settings so we can reset them properly
...
args_per_page = 10    # number of args per page
...
# A thread stop event, We pass an instance of this to each job to let it know to shutdown
...
class FawkesDataCollection:
    def __init__(self, cfg: FawkesConfig, registry: VMRegistry, shutdown_event):
        ...
        try:
            if mode == "local":
                self.local_db = FawkesDB(cfg.db_path)  # For local mode
        ...
        try:
            if mode == "controller":
                self.controller_db = ControllerDB(cfg.controller_db_path)  # For controller mode
                logger.debug(f"Connected to database: {cfg.controller_db_path} in controller mode")
        except Exception as e:
            logger.error("Failed to open controller database, current mode: {mode}")

        self.monitor = ResourceMonitor(cfg, registry)  # Only used in local mode
    ...
    def get_system_metrics(self):
        ...
        if mode == "local":
            ...
            # get_max_vs tells you how many can be spun up with the currently available resources
            # So we add what is running + what it say could be running and get the total 
            ...
            return [
                ...
                ("CPU Usage", "N/A"),  # Controller doesn't track CPU directly
                ("RAM Usage", "N/A"),
                ("", ""),
                ("", ""),
                ("", ""),
    ...
    def get_test_stats(self):
        ...
        if mode == "local":
            ...
            total_jobs = len(jobs)
            total_tc = sum(job[2] or 0 for job in jobs)  # total_testcases
            gen_tc = sum(job[3] or 0 for job in jobs)    # generated_testcases
            
            # Get total execution time from testcases for running jobs
            if total_jobs > 0:
                ...
                total_time /= 1000  # Convert from ms to seconds
            else:
                total_time = 0
            ...
            return [
                ...
                ("Fuzz Rate", "N/A"),  # Workers don’t report this yet
                ("", ""),
                ("", ""),
                ("", ""),
            ]
    ...
    def get_crash_stats(self):
        ...
        if mode == "local":
            ...
            crashes = self.controller_db.conn.execute(
                "SELECT crash_data, COUNT(*) FROM crashes GROUP BY crash_data"
            ).fetchall()
            total = sum(c[1] for c in crashes)
            unique = len(crashes)  # Assuming crash_data is unique per crash type
            # ControllerDB doesn’t store exploitability yet, so we’ll estimate
            high = medium = low = 0  # Placeholder until workers report it
            return [
                ("Crashes", str(total)),
                ("Unique", str(unique)),
                ("[red]High[/]", str(high)),
    ...
    def get_active_jobs(self):
        ...
        if mode == "local":
            ...
            logger.debug(f"Got jod data from sql: {jobs}") 
            job_data = []
            for job in jobs:
                job_id, name, status, vm_count, total_tc = job
                # Fetch crash stats for this job
                crashes = self.local_db._conn.execute(
                    "SELECT exploitability, COUNT(*) FROM crashes WHERE job_id = ? GROUP BY exploitability",
                    (job_id,)
                ).fetchall()
                total_crashes = sum(c[1] for c in crashes)
                exploits = {exp: count for exp, count in crashes if exp}  # Only include non-NULL exploitability
                # Fill missing exploitability categories with 0
                ...
                job_data.append({
                    "id": str(job_id),
                    "name": str(name),
                    "status": str(status.upper()),  # Match test data casing
                    "vms": str(vm_count) or str(0),
                    "total_tc": total_tc or str(0),
            ...
            for job in jobs:
                ...
                name = config.get("name", f"Job {job_id}")
                # Controller doesn’t track VMs directly, use worker count as proxy
                vms = 1 if worker_id else 0  # 1 VM per assigned worker, 0 if unassigned
                total_tc = config.get("total_testcases", 0)  # Assume config might have this
                
                # Fetch crash stats
                crashes = self.controller_db.conn.execute(
                    "SELECT crash_data, COUNT(*) FROM crashes WHERE job_id = ? GROUP BY crash_data",
                    (job_id,)
                ).fetchall()
                total_crashes = sum(c[1] for c in crashes)

                crashes = self.local_db.get_crashes(job_id)
                total_crashes = len(crashes)
                exploits = {
                    "High": sum(1 for c in crashes if c.get("exploitability") == "HIGH"),
                    "Medium": sum(1 for c in crashes if c.get("exploitability") == "MEDIUM"),
                    "Low": sum(1 for c in crashes if c.get("exploitability") == "LOW"),
                    "Unknown": sum(1 for c in crashes if c.get("exploitability") not in ("HIGH", "MEDIUM", "LOW"))
                }
    ...
    def get_crash_feed(self):
        ...
        if mode == "local":
            ...
            crashes = self.controller_db.conn.execute(
                "SELECT crash_id, job_id, worker_id, crash_data, timestamp FROM crashes ORDER BY timestamp DESC LIMIT 5"
            ).fetchall()
            return [
                {
                    "id": str(c[0]),
                    "type": json.loads(c[3]).get("type", "Unknown"),
                    "sig": json.loads(c[3]).get("signature", "N/A")[:8],
                    "exp": f"[{self._exp_color(None)}]N/A",  # No exploitability yet
                    "job": str(c[1] or "N/A"),
                    "path": f"/crashes/{c[0]}.zip"
                }
                for c in crashes
...
def clear_fawkes_dir():
    """
    Delete everything in ~/.fawkes directory.
    """
...
def prompt_user(prompt: str = "> ") -> str:
    """
    Prompt the user with `prompt` and read line-based input in raw/cbreak mode.
    Returns the string the user typed (without the trailing newline).
    
    Because we're in raw mode, pressing Enter typically sends '\r' (ASCII 13).
    We'll accept either '\r' or '\n' to end the line.
    """
    # Print the prompt (no trailing newline) and flush
    ...
    while True:
        # Wait until there's at least 1 char ready (non-blocking check)
        ready, _, _ = select.select([sys.stdin], [], [], 0.1)
        if ready:
            ch = sys.stdin.read(1)
            # if user pressed Enter in raw mode, we might get '\r'
            if ch in ('\r', '\n'):
                # Echo a newline so it looks normal
                sys.stdout.write("\n")
                sys.stdout.flush()
                break
            elif ch == '\x7f':  # handle backspace in a simple way
                # remove last char if there is one
                if buffer_chars:
                    buffer_chars.pop()
            ...
            # No key pressed yet, we can do something else or just loop
            pass
...
def config_table(title: str, row_info: list[tuple[str, str, str]], highlight_idx: int = -1):
    ...
    table.add_column(justify="left", style="bright_cyan", ratio=1)
...
def update_config_body():
    # Slice fields by section (order preserved as defined)
    ...
    # Compute highlight indices using overall index
...
# List of argument entries: (argument, description)
args_help_entries = [
    ("db", "Sqlite3 db file location (e.g., ~/.fawks/fawkes.db)."),
    ("controller db", "Sqlite3 db file location (e.g., ~/.fawks/controller.db)."),
    ...
    ("no headless", "Turn off headless mode default False."),
    ...
    ("workers", "a comma seprated list of IP/Hostnames of the Fuzz Workers")]
...
def update_dashboard():
    ...
    try:
        ...
        job_panels = [job_panel(j) for j in paginated_jobs]
    except Exception as e:
    ...
    combined = Group(
        ...
        Columns(job_panels, equal=True, expand=True)
    )
...
"""
    layout["help"].update(Panel(Align.left(help_text), border_style="bright_blue"))
...
def edit_config_field(display_label, config_key):
    ...
    # To make sure we don't try and configure empty lines
    if not config_key:
        in_edit_mode = False
    ...
    try:
        ...
        logger.debug(f"An error occured in edit_config_field: {e}")
    in_edit_mode = False
...
##############################################################################
def on_press(key):
    global selected_field, current_view, theme_toggle_requested, should_exit, job_page, in_edit_mode, args_page
    ...
    try:
        ...
        if current_view == "configuration":
            ...
            if key == '\r' or key == '\n':
                ...
                termios.tcflush(sys.stdin, termios.TCIOFLUSH) # flush the keyboard buffer or everything breaks
                return False
        ...
        if ch == 's':
            ...
            logger.debug("Recieved kill command stopping jobs...")
            shutdown_event.set()
        elif ch == 'q':
            ...
            for key in reset_config:
                try:
                    setattr(default_config, key, reset_config[key])
                except Exception as e:
                    logger.error(f"An Error occurred setting up cfg: {e}, on key: {key}")
                    continue
            ...
            try:
                ...
                args_page = (args_page + 1) % (max_args_page + 1)
                layout["jobs"].update(args_help_panel(args_page, args_per_page))
            except Exception as e:
                logger.debug(f"An error occured toggeling the help screen: {e}")
        elif ch == 'e':
            should_exit = True
...
def poll_for_keypress():
    # Return a character if available, otherwise return None
...
def main():
    ...
    with console.screen():
        while not should_exit:
            ...
            if in_edit_mode:
                ...
                termios.tcflush(sys.stdin, termios.TCIOFLUSH) # Flush the buffer to get rid of any weird chars laying around
    ...
    # Final cleanup.
    restore_tty() # Restore the tty setting
    termios.tcflush(sys.stdin, termios.TCIOFLUSH) # Flush the buffer 
    console.clear() 
