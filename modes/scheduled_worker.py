"""
Enhanced Scheduler-Aware Worker Mode

Worker that registers with the scheduler and sends heartbeats with load information.
Compatible with the enhanced scheduler-based controller.
...
def get_worker_capabilities():
    """
    Detect worker system capabilities

    Returns:
        Dict with cpu_cores, ram_gb, max_vms
    """
    import psutil
    ...
    # Estimate max VMs based on available resources
    # Conservative estimate: 1 VM per 2 cores, 2GB RAM per VM
    max_vms_cpu = max(cpu_cores // 2, 1)
    max_vms_ram = max(int(ram_gb // 2), 1)
    max_vms = min(max_vms_cpu, max_vms_ram, 32)  # Cap at 32
...
def get_current_load(active_jobs: dict) -> dict:
    """
    Calculate current worker load

    Args:
        active_jobs: Dict of active jobs with their status

    Returns:
        Dict with active_jobs, used_vms, cpu_usage, ram_usage
    """
    import psutil

    total_vms = 0
    for job_id, job_data in active_jobs.items():
        total_vms += job_data.get("status", {}).get("vm_count", 0)
...
def run_scheduled_worker(cfg):
    """
    Run Fawkes in enhanced worker mode with scheduler integration

    This worker mode:
    - Registers with the scheduler
    - Sends periodic heartbeats with load information
    - Handles jobs from the scheduler-based controller
    """
    system_resources = SystemResources()
    logger.info("Entering scheduled worker mode")
    ...
    tags = cfg.get("worker_tags", [])

    logger.info(f"Worker capabilities: {capabilities}")
    logger.info(f"Worker hostname: {hostname}")
    logger.info(f"Worker tags: {tags}")
    ...
    def handle_connection(conn, addr):
        ...
        try:
            ...
            if msg["type"] == "PUSH_JOB":
                ...
                # Respond with current load information
                current_load = get_current_load(active_jobs)
                response = {
                ...
                conn.send(len(response_data).to_bytes(4, byteorder="big"))
                conn.send(response_data)
                logger.debug(f"Sent heartbeat: {current_load}")
